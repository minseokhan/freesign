...
1. **배포 후 브라우저 확인 2가지** — `/ingest` 분석 이벤트가 여전히 들어오는지, 서명 링크가 정상 렌더되는지 (아래 8절).
2. **`TSA_URL` 프로덕션 값 확정** (4절).
3. **6절 (2)(3)(4)** — 전부 "현상 유지/나중" 추천.
4. **Dependabot #8** — rebase 후 초록이면 병합. #7·#9는 별도 작업.
5. (사업 판단이 서면) **Polar 프로덕션 전환** — 4절의 체크리스트 참고.

> 재스캔 high 4건(6-1절)과 CSP nonce(6절 (1))는 전부 수정·배포했습니다.
...
- [ ] 배포 후 브라우저 확인 — 분석 이벤트 수신 · 서명 링크 정상 렌더 (8절)
