...
2. **6절 (2)(3)(4)** — 전부 "현상 유지/나중" 추천. 판단만 남았습니다.
3. **Dependabot #8** — rebase 후 초록이면 병합. #7(vite 7 선행)·#9(tailwind v4)는 별도 작업.
4. (사업 판단이 서면) **Polar 프로덕션 전환** — 4절의 체크리스트 참고.

> 재스캔 high 4건(6-1절)·CSP nonce(6절 (1))·`TSA_URL` 프로덕션 확정(4절)·배포 후 브라우저 확인(8절)은 전부 끝났습니다.
