...
그런데 **DB(원격 Supabase)에는 마이그레이션 0038~0041이 아직 적용되지 않았습니다.** git 커밋만으로는 원격 DB가 바뀌지 않기 때문입니다.

즉 지금 배포된 앱은 이런 상태입니다:

| 기능 | 지금(마이그레이션 전) | 0038~0041 적용 후 |
|---|---|---|
| 데모 데이터 채우기 | ❌ 실패 (`seed_demo_data` 함수가 DB에 없음) | ✅ 정상 |
| 서명 완결 시 TSA 타임스탬프 저장 | ⚠️ 실패하지만 서명 자체는 정상 (best-effort라 로그만 남음) | ✅ 정상 |
| 나머지 기능(계약·인보이스·서명·PDF·결제) | ✅ 정상 | ✅ 정상 |

**데모 시드는 랜딩/온보딩에서만 쓰는 기능이고 나머지 핵심 흐름은 멀쩡합니다.** 그래도 아래 1번을 되도록 빨리 해주세요.
...
## 1. [필수·최우선] 원격 마이그레이션 0038~0041 적용
...
- [ ] 0038 → 0039 → 0040 → 0041 순서로 원격 적용
- [ ] 검증 쿼리 5개 실행해서 기대값 확인
