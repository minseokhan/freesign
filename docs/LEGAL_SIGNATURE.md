...
매듭 v1 간이 서명은 **계약을 무효로 만드는 문제는 없지만**, 분쟁 시 "이 서명이 진짜다"를 서비스가 입증하기 어렵다는 **입증력(증거력) 리스크**가 핵심이다. 코드가 `legalEffect: "none"`으로 명시하고 면책 문구를 노출하는 현재 설계는 이 리스크를 인지하고 격리한 타당한 형태다. v2에서 신원확인·제3자 타임스탬프·완결증명서를 더하면 "법적 효력 있는 서명"에 근접한다.
...
→ 매듭은 사용자가 어떤 계약이든 담을 수 있으므로, **"모든 계약에 법적 효력을 보장한다"는 뉘앙스는 절대 주면 안 된다.**
