# 매듭 Pro 3-기능 통합 구현 계획 — 미수금 독촉 · 반복 인보이스 · AI 계약 인사이트

> ✅ **구현 완료(2026-07-25, 커밋 `fb9e660` + 후속 수정)** — 마이그레이션 `0027`~`0031` 원격 적용 완료. 결정 기록은 `docs/ADR.md` ADR-011(및 ADR-010 갱신), 현재 구조는 `docs/ARCHITECTURE.md`·`docs/DATABASE.md`.
> 아래는 실행 당시의 계획 원본이므로, 최종 구현과 세부가 다를 수 있다(정본은 코드·마이그레이션).

## Context (왜)

매듭 유료화(Polar 월 구독)는 이미 구현·sandbox E2E 검증까지 끝났다. 하지만 가격 전략을 점검하며 **구조적 문제**가 드러났다:
