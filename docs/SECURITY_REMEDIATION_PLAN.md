...
## 배치 1 — High, 코드 국소 (진행 중)

| # | 심각도 | 항목 | 파일 |
|---|---|---|---|
| 9 | High | Next.js 15.5.20 → 15.5.22 (공개 취약점 8건) | `package.json` |
| 15 · 27 | High | posthog가 `$current_url`로 서명 토큰 전송 | `instrumentation-client.ts` |
...
| # | 심각도 | 항목 |
|---|---|---|
| 22 | High | 독촉 크론에 Pro 게이트·배치 상한 없음 → 무제한 Claude 호출 |
| 31 · 6 | High/Medium | `contracts`·`invoices` 컬럼 무제한 UPDATE → 서버 소유 필드 변조 |
| 30 · 36 | High | `send_signature_request_with_event` DEFINER 전환 + 증거 테이블 `revoke insert` |
...
| 23 | 폐기된 단독 서명 엔드포인트 잔존 |
| 28 | Supabase 세션 쿠키 HttpOnly·Secure |
| 32 | `is_demo` 클라이언트 쓰기 → 데모 삭제 정책 악용 |
| 37 · 38 · 46 | 크론 실패·감사 INSERT 실패 무기록 |
| 41 · 42 · 43 · 44 · 45 | fail-open 게이트 · 발송 실패의 성공 처리 · 원본 에러 노출 |
