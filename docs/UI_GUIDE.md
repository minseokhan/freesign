...
2. **뉴트럴 + 남색 1색.** 차분한 슬레이트 뉴트럴 위에 브랜드 남색 1색만 액션·강조에 쓴다. 색을 아낄수록 정산 신호색(완료/대기/미수)이 살아난다.
...
| 보라/인디고 브랜드 색상 | "AI = 보라색" 클리셰 (우리는 남색 1색) |
...
### 브랜드 포인트 (남색 1색)
정본은 `tailwind.config.ts`의 `colors.brand`다. 매듭 로고의 워드마크 남색과 심볼 링 블루에서 가져왔다.

| 용도 | 토큰 | 값 |
|------|------|------|
| Primary(버튼·링크·활성 탭) | `brand.primary` | #2b3587 |
| Primary hover | `brand.hover` | #212a6c |
| 포커스 링 | `brand.ring` | #4e61f6, 2px |
| 포인트 배경(선택·배지·활성 내비) | `brand.point` | #eef0fb |
...
> 시맨틱 색은 **상태 배지·금액 강조·대시보드 하이라이트**에만. 일반 UI 강조는 남색 포인트만 사용해 신호를 희석하지 않는다. 상태는 색 + **텍스트 라벨 병기**(색각 이상 대비, §접근성).
...
예) "아직 매듭지은 계약이 없어요" + [계약 매듭짓기] / 첫 진입 → [데모 데이터 채우기]
```
### 이력 타임라인 (증빙 체인)
```
좌측 세로선 + 노드(단일 브랜드 남색 고정) + 시각(text-slate-500) + 설명
