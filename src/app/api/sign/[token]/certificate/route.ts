...
import { parseCertificateData } from "@/lib/contracts/public-sign";
...
// 상대방 교부용 완결증명서 — completed 토큰 소지자에게만 응답한다(anon DEFINER RPC).
export async function GET(request: Request, context: RouteContext) {
  ...
  const certificate = parseCertificateData(data, {
    tsaUrl: getTimestampEnv().TSA_URL ?? null,
  });

  if (!certificate) {
    return NextResponse.json(
      { error: "완결된 서명 요청만 완결증명서를 내려받을 수 있습니다." },
      { status: 404 },
    );
  }
  ...
  return new NextResponse(new Uint8Array(pdfBuffer), {
    headers: {
      ...
      "content-disposition": `inline; filename="certificate-${certificate.contractId}.pdf"`,
