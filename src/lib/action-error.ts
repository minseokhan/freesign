 ...
 * 스키마 내부 구조를 노출하므로 클라이언트에 담지 않는다. 상세는 서버 로그와
 * PostHog 에러 트래킹으로만 남기고, 응답에는 고정된 일반 메시지만 반환한다.
 * 사용자가 조치 가능한 입력 검증 오류는 각 액션의 validationError로 별도 처리한다.
 */
export async function dbError(
  ...
  if (error?.message) {
    ...
    await captureServerException(new Error(error.message), undefined, {
